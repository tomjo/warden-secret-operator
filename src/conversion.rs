...
fn convert(mut request_obj: Value, desired_api_version: &str) -> Result<Value, Status> {
    let current_api_version: String = request_obj
        .as_object()
        .map(|x| x.get("apiVersion"))
        .flatten()
        .map(|x| x.as_str())
        .flatten()
        .ok_or(Status::failure(
            "No apiVersion in request",
            "ConversionRequestInvalid",
        ))?
        .to_string();
    ...
    if current_api_version == desired_api_version {
        return Err(Status::failure(
            format!("Already at desired API version {}", desired_api_version).as_ref(),
            "AlreadyAtDesiredAPIVersion",
        ));
    }
    let orig_req_obj = request_obj.to_string();
    match desired_api_version {
        "tomjo.net/v1" => match current_api_version.as_str() {
            "tomjo.net/v2" => {
                ...
                request_obj["status"]
                    .as_object_mut()
                    .and_then(|m| m.remove("observedGeneration"));
                request_obj["status"]
                    .as_object_mut()
                    .and_then(|m| m.remove("startTime"));
            ...
            _ => Err(Status::failure(
                format!(
                    "Unsupported API conversion from {} to {}",
                    current_api_version, desired_api_version
                )
                .as_ref(),
                "UnsupportedAPIConversion",
            )),
        },
        "tomjo.net/v2" => match current_api_version.as_str() {
            "tomjo.net/v1" => {
                request_obj["apiVersion"] = Value::String("tomjo.net/v2".to_string());
                let orig_start_time = request_obj["status"]["start_time"].clone();
                request_obj["status"]["startTime"] = if orig_start_time.is_null() {
                    Value::Null
                } else {
                ...
                request_obj["status"]
                    .as_object_mut()
                    .and_then(|m| m.remove("phase"));
                request_obj["status"]
                    .as_object_mut()
                    .and_then(|m| m.remove("start_time"));
        ...
        _ => Err(Status::failure(
            format!("Unsupported API version {}", desired_api_version).as_ref(),
